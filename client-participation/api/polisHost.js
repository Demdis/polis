(function() {
  ...
  function createPolisIframe(parent, o) {
    ...
    iframe.src = "https://polis.demdis.sk/"+ path.join("/");
  ...
  if (firstRun) {
    window.addEventListener("message", function(event) {
      ...
      if (!event.origin.match(/polis.demdis.sk$/)) {
        return;
      }
      ...
      if (event.data === "cookieRedirect" && cookiesEnabledAtTopLevel()) {
        ...
        window.location = "https://polis.demdis.sk/api/v3/launchPrep?dest=" + encodeReturnUrl(window.location+"");
      }
    }, false);
  }
